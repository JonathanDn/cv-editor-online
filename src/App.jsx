...
const OLD_TEMPLATE_LOCAL_STORAGE_KEY = 'cvEditorData_v2';
const CURRENT_LOCAL_CONTENT_KEY = 'cvEditorCurrentContent_v1';
...
const normalizeOldSchema = (schema = {}) => {
  const defaultSchema = createDefaultCvSchema();
  ...
  return {
    ...
    additional: {
      ...defaultSchema.additional,
      ...(schema.additional || {}),
    },
    contacts: Array.isArray(schema.contacts) ? schema.contacts : defaultSchema.contacts,
    education: Array.isArray(schema.education) ? schema.education : defaultSchema.education,
    awards: Array.isArray(schema.awards) ? schema.awards : defaultSchema.awards,
    languages: typeof schema.languages === 'string' ? schema.languages : defaultSchema.languages,
    leftSections: Array.isArray(schema.leftSections) ? schema.leftSections : [],
    experience: Array.isArray(schema.experience) ? schema.experience : defaultSchema.experience,
  };
};

const renderOldSchemaToCurrentHtml = (rawSchema) => {
  const schema = normalizeOldSchema(rawSchema);

  return `
    <header class="profile-container hero" data-testid="profile-container">
      <h2 class="hero-name">
        <span class="hero-first">${escapeHtml(schema.profile.firstName)}</span>
        <span class="hero-last">${escapeHtml(schema.profile.lastName)}</span>
      </h2>
      <p class="hero-title">${escapeHtml(schema.profile.title)}</p>
      <p class="hero-summary">${escapeHtml(schema.profile.summary)}</p>
    </header>

    <section class="content-grid" data-testid="columns-container">
      <aside class="contact-container left-column" data-testid="contact-container">
        <section class="panel">
          <h3>Contact</h3>
          ${schema.contacts.map((item) => `<p>${escapeHtml(item)}</p>`).join('')}
        </section>

        <section class="panel">
          <h3>Education</h3>
          ${schema.education.map((item) => `
            <div class="item">
              <h4>${escapeHtml(item.qualification)}</h4>
        ...
        <section class="panel languages-section" data-testid="languages-section">
          <h3>Languages</h3>
          <p>${escapeHtml(schema.languages)}</p>
        </section>

        ${schema.leftSections.map((section) => `
      ...
      <section class="experience-container right-column" data-testid="experience-container">
        <section class="panel">
          <h3>Experience</h3>
          ${schema.experience.map((job) => `
            <div class="job">
    ...
    <section class="panel additional-section" data-testid="additional-section">
      <h3>${escapeHtml(schema.additional.title)}</h3>
      <p>${escapeHtml(schema.additional.text)}</p>
    </section>
  `.trim();
};

function readLocalCurrentContent() {
  const raw = window.localStorage.getItem(CURRENT_LOCAL_CONTENT_KEY);
...
function writeLocalCurrentContent(html) {
  window.localStorage.setItem(
      CURRENT_LOCAL_CONTENT_KEY,
      JSON.stringify({
        html,
...
function migrateOldLocalStorageToCurrentContent() {
  const existingCurrentContent = readLocalCurrentContent();

  if (existingCurrentContent) return existingCurrentContent;

  const oldStructuredRaw = window.localStorage.getItem(OLD_TEMPLATE_LOCAL_STORAGE_KEY);

  if (oldStructuredRaw) {
    try {
      const oldStructured = JSON.parse(oldStructuredRaw);

      if (oldStructured?.schema) {
        const html = renderOldSchemaToCurrentHtml(oldStructured.schema);
        writeLocalCurrentContent(html);

        return { html };
      }
    } catch {
      // Continue to legacy localStorage migration.
    }
  }
  ...
  if (legacyRaw) {
    try {
      ...
      if (typeof legacy?.documentHtml === 'string') {
        writeLocalCurrentContent(legacy.documentHtml);

        return { html: legacy.documentHtml };
      }
    } catch {
      // Continue to default template fallback.
    }
  }

  const html = renderOldSchemaToCurrentHtml(createDefaultCvSchema());
  writeLocalCurrentContent(html);

  return { html };
}

function CvListPage({ onCompare, targetCvId, onSetTargetCv, onCreateCv }) {
  ...
  useEffect(() => {
    setPage(1);
  }, [status, sort, tagFilter, folderFilter]);
  ...
  const runBulkAction = async (action) => {
    ...
    if (!window.confirm(`${action[0].toUpperCase()}${action.slice(1)} ${selectedIds.length} CV(s)?`)) {
      return;
    }
  ...
  const runRowAction = async (cv, action) => {
    if (action === 'Open') {
      window.location.search = `?id=${encodeURIComponent(cv.id)}`;
      return;
    }
    ...
    const res = await fetch(`/api/cvs/${cv.id}/${actionKey}`, {
      method: 'POST',
      headers: authHeaders,
    });
  ...
  return (
      <section className="cvs-page">
        <div className="cvs-header">
          ...
          <button className="primary-btn" type="button" onClick={onCreateCv}>
        ...
        <div className="status-chips">
          {TABS.map((tab) => (
              <button
                  key={tab.key}
                  className={`chip${status === tab.key ? ' active' : ''}`}
                  type="button"
        ...
        <div className="status-chips">
          ...
          <input
              ...
              value={folderFilter}
              onChange={(e) => setFolderFilter(e.target.value.trim())}
          />
        ...
        <div className="bulk-row">
          <label>
            <input
                ...
                onChange={() => setSelectedIds(allVisibleSelected ? [] : rows.map((cv) => cv.id))}
            />
            {' '}
            Select page
          </label>

          <button type="button" className="secondary-btn" onClick={() => runBulkAction('archive')} disabled={!selectedIds.length}>
            Archive
          </button>

          <button type="button" className="secondary-btn" onClick={() => runBulkAction('delete')} disabled={!selectedIds.length}>
            Delete
          </button>

          <button type="button" className="secondary-btn" onClick={() => runBulkAction('restore')} disabled={!selectedIds.length}>
            Restore
          </button>
        </div>

        {isLoading && (
            <div className="cvs-list skeleton-list" aria-busy="true">
              {Array.from({ length: 4 }).map((_, idx) => (
                  <div className="cv-row skeleton" key={idx} />
              ))}
            </div>
        )}
        ...
        {!isLoading && error && (
            <div className="error-state">
              <p>Failed to load CVs: {error}</p>
              <button type="button" className="secondary-btn" onClick={loadCvs}>Retry</button>
            </div>
        )}
        ...
        {!isLoading && !error && rows.length > 0 && (
            <div className="cvs-list">
              {rows.map((cv) => (
                  <article className="cv-row" key={cv.id}>
                    ...
                    <div>
                      <h3>{cv.title}</h3>
                      <p>{roleCompanyLabel(cv)}</p>
                      <small>Updated {formatUpdatedAt(cv.updatedAt)}</small>
                      {targetCvId === cv.id && <small>Target CV selected</small>}
                    </div>

                    <div className="row-actions">
                      <button type="button" onClick={() => onCompare(cv.id)}>Compare</button>
                      <button type="button" onClick={() => onSetTargetCv(cv.id)}>Set as target CV</button>

                      {(status === 'deleted' ? DELETED_ROW_ACTIONS : ROW_ACTIONS).map((action) => (
                          <button key={action} type="button" onClick={() => runRowAction(cv, action)}>
                            {action}
                          </button>
                      ))}
                    </div>
...
function App() {
  ...
  const [editorHtml, setEditorHtml] = useState('<h2>CV Editor</h2>');
  ...
  const loadCurrentCv = useCallback(async () => {
    ...
    const normalizedContent = data.content_json && typeof data.content_json === 'object'
        ? data.content_json
        : {};

    const html = parseContentJsonToHtml(data.content_json);

    setCvContentJson(normalizedContent);
    setCvRevision(data.updatedAt || data.revision || null);
    setEditorHtml(html || '<h2>CV Editor</h2>');
  ...
  const saveCv = useCallback(async (mode = 'manual') => {
    ...
    if (res.ok) {
      const data = payload?.data || {};
      const contentJson = data.content_json && typeof data.content_json === 'object'
          ? data.content_json
          : { html: nextContent };
  ...
  const saveEditorToMyCvs = useCallback(async () => {
    const html = cvRef.current?.innerHTML || editorHtml || '<h2>CV Editor</h2>';

    const defaultTitle = `CV ${new Date().toLocaleDateString()}`;
    const title = window.prompt('Save this CV to My CVs as:', defaultTitle)?.trim();
    ...
    setAutosaveStatus('Saving to My CVs…');
    ...
    try {
      ...
      if (!res.ok) {
        setAutosaveStatus(compactError('Could not save CV to My CVs.', payload));
        return;
      }

      const createdCv = payload?.data;

      writeLocalCurrentContent(html);

      setImportMessage(`Saved "${createdCv?.title || title}" to My CVs.`);
      setAutosaveStatus('Saved to My CVs');

      if (createdCv?.id && window.confirm('Saved to My CVs. Open the saved CV now?')) {
        window.location.search = `?id=${encodeURIComponent(createdCv.id)}`;
      }
    } catch {
      setAutosaveStatus('Could not save CV to My CVs.');
    }
  }, [editorHtml]);

  const createFirstCv = useCallback(async () => {
    const migratedContent = migrateOldLocalStorageToCurrentContent();
    const html = migratedContent.html || renderOldSchemaToCurrentHtml(createDefaultCvSchema());

    const defaultTitle = `CV ${new Date().toLocaleDateString()}`;
    const title = window.prompt('Name your new CV:', defaultTitle)?.trim();
    ...
    try {
      ...
      if (!res.ok) {
        setImportMessage(compactError('Could not create CV.', payload));
        setAutosaveStatus('Create failed');
        return;
      }
      ...
      setRoute('editor');
      setEditorHtml(html);
      setCvContentJson({ html });
      setAutosaveStatus('Saved locally');
    } catch {
      setImportMessage('Could not create CV.');
      setAutosaveStatus('Create failed');
    }
  }, []);
  ...
  const runCompare = useCallback(async (baseCvId, leftSnapshotId = 'current', rightSnapshotId = 'current') => {
    ...
    try {
      const params = new URLSearchParams({
        left: leftSnapshotId,
        right: rightSnapshotId,
      });

      const res = await fetch(`/api/cvs/${baseCvId}/compare?${params.toString()}`, {
        headers: authHeaders,
      });
  ...
  const restoreSnapshot = useCallback(async (snapshotId) => {
    ...
    const html = parseContentJsonToHtml(contentJson);
  ...
  const exportJson = useCallback(async () => {
    ...
    if (!res.ok) {
      setImportMessage(compactError('Export failed.', payload));
      return;
    }
  ...
  useEffect(() => {
    ...
    setCvContentJson(migratedContent);
    setEditorHtml(migratedContent.html || '<h2>CV Editor</h2>');
    setAutosaveStatus('Saved locally');
    setHasUnsavedChanges(false);
  }, [cvId]);
  ...
  return (
      <main className="page">
        <header className="app-nav">
          ...
          <button
              type="button"
              className={route === 'my-cvs' ? 'active' : ''}
              onClick={() => goToRoute('my-cvs')}
          >
            My CVs
          </button>
        </header>
        ...
        {route === 'my-cvs' ? (
            <CvListPage
                ...
                onSetTargetCv={setTargetCvId}
                onRestoreDeleted={restoreDeletedCv}
                onCreateCv={createFirstCv}
            />
        ) : (
            <section className="cv-shell">
              <button type="button" className="save-pdf-button" onClick={() => window.print()}>
                <MdOutlineSaveAs />
              </button>
              ...
              <div className="editor-toolbar">
                <button
                    ...
                    disabled={!cvId || isLoadingSnapshots}
                >
                  {isLoadingSnapshots ? 'Loading snapshots…' : 'Snapshots'}
                </button>

                <button
                    type="button"
                    className="secondary-btn"
                    onClick={() => runCompare(cvId)}
                    disabled={!cvId || isComparing}
                >
                  {isComparing ? 'Comparing…' : 'Compare'}
                </button>

                <button type="button" className="secondary-btn" onClick={saveEditorToMyCvs}>
                  Save to My CVs
                </button>

                <button type="button" className="secondary-btn" onClick={exportJson} disabled={!cvId}>
                  Export JSON
                </button>
              ...
              <p className="helper-text">
                {cvId
                    ? 'Import always creates a new CV copy and does not overwrite your current document.'
                    : 'Loaded from this browser’s local CV template.'}
              </p>
              ...
              {showConflictModal && (
                  <div className="conflict-backdrop">
                    <div className="conflict-modal" data-testid="conflict-reload-modal">
                      <h3>{CONFLICT_TITLE}</h3>
                      <span hidden>{RELOAD_MODAL_LABEL}</span>
                      ...
                      <div className="conflict-actions">
                        <button
                            type="button"
                            className="secondary-btn"
                            data-testid="conflict-reload-action"
                        ...
                        <button
                            type="button"
                            className="secondary-btn"
                            data-testid="conflict-close-action"
                      ...
                      {snapshots.length === 0 ? (
                          ...
                          <ul>
                            {snapshots.map((snapshot) => (
                                <li key={snapshot.id}>
                                  <span>{formatUpdatedAt(snapshot.createdAt)}</span>

                                  <button
                                      type="button"
                                      className="secondary-btn"
                                      onClick={() => runCompare(cvId, snapshot.id, 'current')}
                                  >
                                    Compare
                                  </button>
              ...
              {compareData && (
                  <div className="compare-grid">
                    {sections.map((section) => (
                        <div className="compare-row" key={section}>
                          <h4>{section}</h4>

                          <div className="compare-cols">
                            <div>
                              <pre>{compareData.normalized.left?.[section] || ''}</pre>
                              <button type="button" onClick={() => applySection(section, 'left')}>
                                Apply left section
                              </button>
                            </div>

                            <div>
                              <pre>{compareData.normalized.right?.[section] || ''}</pre>
                              <button type="button" onClick={() => applySection(section, 'right')}>
                                Apply right section
                              </button>
                            </div>
                          </div>
